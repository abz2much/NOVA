#!/usr/bin/env python3
"""
Nova AIO bootstrap orchestrator.
Completes post-install steps programmatically so the user touches nothing:
  ...
  2. Download the Nova voice model (integrity-verified)
  ...
  5. Create the Nova Assist pipeline (best-effort — HA WS schemas vary)
  6. Set Nova pipeline as default
...
def _try_quality(quality: str) -> bool:
    ...
    HF file structure (as of 2026-04): en/en_GB/nova/{quality}/nova-{quality}.onnx
    Local file structure (what Piper/Wyoming expects): en_GB-nova-{quality}.onnx
    So we download under the HF name and RENAME to the Piper name.
    """
    onnx = PIPER_DIR / f"en_GB-nova-{quality}.onnx"
    json_file = PIPER_DIR / f"en_GB-nova-{quality}.onnx.json"
...
def download_nova_voice(quality: str) -> bool:
    """
    Download + verify the Nova voice model.
    ...
    for q in ("high", "medium"):
        onnx = PIPER_DIR / f"en_GB-nova-{q}.onnx"
        json_file = PIPER_DIR / f"en_GB-nova-{q}.onnx.json"
        if onnx.exists() and json_file.exists() and onnx.stat().st_size > MIN_ONNX_SIZE:
            log(f"Voice model already present: en_GB-nova-{q} ({onnx.stat().st_size} bytes).")
            return True
    ...
    log(f"Manual install: https://huggingface.co/jgkawell/nova/tree/main/en/en_GB/nova/{quality}", "ERROR")
    log(f"  → copy both files to {PIPER_DIR}/  (rename to en_GB-nova-{quality}.onnx[.json])", "ERROR")
    return False
...
async def create_nova_pipeline(voice_quality: str) -> bool:
    """
    Build the Nova pipeline and set it as preferred.
    ...
    try:
        ...
        if not nova_conv:
            log(
                "Nova conversation agent not found in conversation/agent/list.",
                "WARN",
            )
        ...
        log(f"Pipeline: conv={nova_conv} stt={stt_engine} tts={tts_engine}/{tts_voice}")

        payload = {
            "name": "Nova",
            "conversation_engine": nova_conv,
        ...
        existing = next((p for p in pipelines if p.get("name") == "Nova"), None)
        if existing:
            pipeline_id = existing.get("pipeline_id") or existing.get("id")
            log(f"Nova pipeline exists ({pipeline_id}) — updating.")
        ...
        log(f"Nova pipeline ({pipeline_id}) is now default.")
        return True
...
STATUS_FILE = Path("/tmp/nova_bootstrap_status.json")
...
async def main() -> int:
    honorific     = os.environ.get("NOVA_HONORIFIC", "sir")
    voice_quality = os.environ.get("NOVA_VOICE_QUALITY", "medium")
    tts_provider  = os.environ.get("NOVA_TTS_PROVIDER", "piper_nova")
    auto_pipeline = os.environ.get("NOVA_AUTO_PIPELINE", "true").lower() == "true"
    ...
    if tts_provider == "piper_nova":
        log("─" * 60)
        log("Phase 2/5: Download Nova voice model")
        voice_ok = download_nova_voice(voice_quality)
    else:
    ...
    if tts_provider == "piper_nova" and voice_ok:
        log("─" * 60)
        ...
        if addon_restart("core_piper", "Piper TTS"):
            if wait_for_addon_state("core_piper", "started", tries=30, sleep_s=2):
                log("Piper restarted with Nova voice loaded.")
            else:
    ...
    # Wait for Nova custom component to register its conversation agent.
    # On first install HA has to pip-install groq, which can take 30-60s.
    log("Waiting for Nova conversation agent to register (up to 120s)...")
    agent_found = False
    last_agents = []
    for attempt in range(40):  # 40 × 3s = 120s
        try:
            ...
            if res:
                ...
                if any("nova" in a.lower() for a in last_agents):
                    log(f"Nova conversation agent ready (after {attempt * 3}s).")
                    agent_found = True
                    break
        except Exception:
            pass
    ...
    if not agent_found:
        log("Nova conversation agent did not register within 120s timeout.", "WARN")
        ...
        log("  2. Python error in custom_components/nova — check logs for", "WARN")
        log("     'Error importing custom_components.nova' or similar", "WARN")
        log("  3. nova_config.json has a malformed field", "WARN")
        log("Try: Settings → System → Logs, search 'nova' for the actual error.", "WARN")
    ...
    if auto_pipeline and agent_found:
        log("─" * 60)
        log("Phase 5/5: Create Nova Assist pipeline")
        pipeline_ok = await create_nova_pipeline(voice_quality)
        if not pipeline_ok:
            log(
            ...
            log(f"  Conversation: Nova     TTS: piper / en_GB-nova-{voice_quality}", "INFO")
            log("  STT: faster-whisper      Wake word: hey_nova", "INFO")
    elif auto_pipeline and not agent_found:
        log("Phase 5/5: Skipped — Nova agent not registered, pipeline would fail.", "WARN")
    else:
