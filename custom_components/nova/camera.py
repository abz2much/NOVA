"""
Nova — Camera image analysis via Groq vision.
...
async def _reason_about_scene(
    ...
    system = (
        "You are Nova's camera-reasoning module. You are given a description of "
...
def _disabled_cameras() -> set:
    ...
    the cameras Nova must NOT use (v7.0.0)."""
    try:
        from . import nova_config
        val = nova_config.get("disabled_cameras")
...
def active_camera_states(hass) -> list:
    ...
    all disabled => none. The single gate for every place Nova enumerates
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if len(data) > MIN_IMAGE_SIZE:
                                _LOGGER.debug(
                                    "Nova: got Frigate event snapshot (%d bytes)",
                                    len(data),
                                )
                                return data
                        else:
                            _LOGGER.debug(
                                "Nova: Frigate event snapshot returned %d", resp.status
                            )
                except Exception as exc:
                    _LOGGER.debug("Nova: Frigate event snapshot failed: %s", exc)
...
async def _fetch_nest_event_image(
    ...
    age = (datetime.now(timezone.utc).replace(tzinfo=None) - cached["ts"]).total_seconds()
    if age >= EVENT_FRESH_SECONDS:
        _LOGGER.debug("Nova: Nest event too old (%ds) for %s", int(age), entity_id)
        return None
                        ...
                        if media_store:
                            ...
                            if media and hasattr(media, "contents"):
                                ...
                                if data and len(data) > MIN_IMAGE_SIZE:
                                    _LOGGER.debug(
                                        "Nova: got Nest media via Python API (%d bytes)",
                                        len(data),
                                    )
                                    return data
                except Exception as inner_exc:
                    _LOGGER.debug("Nova: Nest Python API error: %s", inner_exc)
    except Exception as exc:
        _LOGGER.debug("Nova: Nest hass.data access failed: %s", exc)

    _LOGGER.debug(
        "Nova: Nest event media not retrievable via in-process API — "
...
async def _prewarm_stream(hass: HomeAssistant, entity_id: str, settle: float = 2.5) -> bool:
    ...
    try:
        ...
        _LOGGER.debug("Nova: stream pre-warm failed for %s: %s", entity_id, exc)
    return False
...
async def probe_camera(hass: HomeAssistant, entity_id: str) -> dict:
    ...
    if not out["available"]:
        out["verdict"] = ("entity missing from HA" if not st
                               ...
                               "integration first (nothing Nova can fetch)")
...
def display_name(entity_id: str, friendly: Optional[str],
    ...
    """Nova-only camera display name (v6.48.0): the `camera_names` runtime
...
async def _get_best_image(hass: HomeAssistant, entity_id: str) -> Optional[bytes]:
    ...
    if backend:
        try:
            ...
            if data:
                _LOGGER.debug(
                    "Nova: got image via backend '%s' (%d bytes)",
                    backend.name, len(data),
                )
                return data
            _LOGGER.debug(
                "Nova: backend '%s' returned no image — falling back to standard snapshot",
                backend.name,
            )
        except Exception as exc:
            _LOGGER.debug(
                "Nova: backend '%s' error: %s — falling back",
                backend.name, exc,
            )
    ...
    try:
        ...
        _LOGGER.debug("Nova: first snapshot attempt failed for %s: %s", entity_id, exc)
    ...
    _LOGGER.info(
        "Nova: %s snapshot blank/tiny/empty — waking stream and retrying", entity_id,
    )
    await _prewarm_stream(hass, entity_id)
    try:
        ...
        if tiny_first:
            _LOGGER.debug(
                "Nova: %s wake retry no better — returning tiny first frame "
        ...
        _LOGGER.warning(
            "Nova: %s still blank/empty after stream wake (%d bytes) — "
            "on-demand WebRTC camera with no active stream?",
            entity_id, size,
        )
    except Exception as exc:
        _LOGGER.error("Nova: snapshot retry failed for %s: %s", entity_id, exc)
...
async def _capture_frame_sequence(
    ...
    for i in range(count):
        try:
            ...
            _LOGGER.debug("Nova: frame %d capture failed for %s: %s", i, entity_id, exc)
        if i < count - 1:
    ...
    _LOGGER.debug("Nova: captured %d frame(s) from %s", len(frames), entity_id)
    return frames
...
async def async_analyze_camera(
    ...
    and incorporates the result into Nova's awareness. When gate_announce is
    ...
    _LOGGER.info("Nova: analyzing %s (%s, source=%s)", entity_id, camera_name, integration)
    ...
    if not frame_bytes:
        ...
        try:
            ...
            nova_log("CAMERA", f"{camera_name}: no usable image (WebRTC/offline?)")
        except Exception:
            pass
        if announce:
    ...
    if is_clip:
        ...
        task = (
            ...
            f"Describe what you see clearly and concisely — as Nova would. "
    ...
    try:
        ...
        if _vision_model_rejects_images(exc):
            ...
            _LOGGER.error("Nova vision (%s/%s): %s [raw: %s]",
                          vision_provider, vision_model, friendly, exc)
            try:
                from .websocket import nova_log
                nova_log("CAMERA", f"{camera_name}: {friendly}")
            except Exception:
                pass
            return {"success": False, "error": friendly, "camera": camera_name}
        _LOGGER.error("Nova vision error (%s/%s): %s", vision_provider, vision_model, exc)
        try:
            from .websocket import nova_log
            nova_log("CAMERA", f"{camera_name}: vision error ({vision_provider}/{vision_model}) — {exc}")
        except Exception:
            pass
    ...
    try:
        ...
        nova_log("CAMERA", f"{camera_name} [{judgment['category']}]{flag}: {summary}")
    except Exception:
        pass

    # ── Incorporate into the rest of Nova ───────────────────────────────────
...
def _nest_device_to_camera(hass: HomeAssistant, nest_device_id: str) -> Optional[str]:
    ...
    try:
        ...
        _LOGGER.debug("Nova: _nest_device_to_camera error: %s", exc)
...
def _handle_nest_event(hass: HomeAssistant, event: Event) -> None:
    ...
    try:
        ...
        _LOGGER.debug("Nova: cached Nest event %s for %s", event_id, entity_id)
        try:
            hass.bus.async_fire("nova_camera_event", {
                "entity_id": entity_id,
                "source": "nest",
        ...
        _LOGGER.debug("Nova: error caching Nest event: %s", exc)
...
def _handle_frigate_event(hass: HomeAssistant, event: Event) -> None:
    ...
    try:
        ...
        if data.get("type") == "new":
            ...
            _LOGGER.debug("Nova: cached Frigate event %s for %s", event_id, entity_id)
...
def register_event_listeners(hass: HomeAssistant) -> list:
    ...
    try:
        ...
        _LOGGER.debug("Nova: could not subscribe to nest_event: %s", exc)
...
async def _analyze_doorbell_press(
    ...
    if res.get("success"):
        ...
        try:
            ...
            _LOGGER.debug("Nova: doorbell training log error: %s", exc)
        ...
        try:
            ...
            _LOGGER.debug("Nova: doorbell package note error: %s", exc)

    _LOGGER.info(
        "Nova doorbell press on %s analysed (%s frames) → notable=%s",
...
async def async_visitor_observation(
    ...
    if res.get("success"):
        try:
            ...
            _LOGGER.debug("Nova: visitor learning log error: %s", exc)
        _LOGGER.info("Nova visitor observation on %s logged (silent)", entity_id)
