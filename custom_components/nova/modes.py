"""
Nova Directive Layer — operational modes (v6.61.0).

The generalization of the Lockdown state machine. Instead of a single formal
state, Nova supports named operational modes that each shift its whole
...
Design, mirroring the proven lockdown state pattern:
  - One active mode at a time (default: "normal"), persisted atomically to
    /config/nova/mode_state.json so it survives reboots/reloads.
        ...
        if os.path.exists(MODE_STATE_PATH):
            ...
            if mode in _all_modes():
                ...
                if mode != DEFAULT_MODE:
                    _LOGGER.info("Nova mode RESTORED: %s", mode)
...
def set_mode(name: str, reason: str = "") -> dict:
    ...
    _persist()
    _LOGGER.info("Nova mode → %s%s", key, f" ({reason})" if reason else "")
...
def auto_enabled() -> bool:
    """Whether Nova may switch operational mode on its own. Users who prefer a
