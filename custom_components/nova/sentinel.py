"""Nova — Sentinel: proactive entity monitoring and anomaly alerting."""
...
class NovaSentinel:
    """Monitors entity states and triggers Nova announcements on anomalies."""
    ...
    def _tts_entity(self) -> str | None:
        if not self._entry:
            return None
        configured = nova_config.runtime_get(
    ...
    def _speakers(self) -> list[str]:
        ...
        broadcast_group = nova_config.runtime_get(
            self.hass, self._entry, "broadcast_group", "") or None
        announcement_speakers = nova_config.runtime_get(
    ...
    async def async_start(self) -> None:
        ...
        _LOGGER.info("Nova Sentinel active — watching %d entities (auto-enroll enabled)",
    ...
    def _refresh_entities(self, now) -> None:  # noqa: ARG002
        ...
        if added or removed:
            ...
            if added:
                _LOGGER.info("Nova Sentinel: enrolled %d new entities: %s",
                             len(added), ", ".join(list(added)[:5]))
            if removed:
                _LOGGER.info("Nova Sentinel: de-enrolled %d entities", len(removed))
    ...
    async def async_stop(self) -> None:
        ...
        self._unsubs.clear()
        self._active = False
        _LOGGER.info("Nova Sentinel stopped.")
    ...
    async def _announce_rule(self, entity_id: str, rule: dict, minutes: int) -> None:
        ...
        if self._entry:
            announcements_enabled = bool(nova_config.runtime_get(
                self.hass, self._entry, "announcements_enabled", True))
            sentinel_enabled = bool(nova_config.runtime_get(
            ...
            try:
                disabled_raw = nova_config.runtime_get(
