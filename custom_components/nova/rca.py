"""
Nova root cause analysis — answering *why* something happened, not just that
it did.

Evidence comes from history Nova already keeps (all local, no network):
  • state_changes + commands  in /config/nova/patterns.db   (the StateLogger)
  • activity_log              in /config/nova/conversations.db (Nova's own
...
and ranks candidate causes with simple, *explainable* heuristics:
  ...
  nova       Nova itself acted on it (or its room) shortly before
...
NEAR_WINDOW_SECS = 300         # commands / Nova actions "shortly before"
...
def candidates(ev: dict) -> list[dict]:
    ...
    for act in ev["actions"]:
        ...
        if act.get("entity_id") == eid or any(t in msg for t in toks) \
            ...
            out.append({"kind": "nova", "confidence": 0.8,
                        "cause": f"Nova acted {int(secs)}s earlier: "
    ...
    if not out:
        out.append({"kind": "unknown", "confidence": 0.0,
                    ...
                    "evidence": "no attributed trigger, upstream failure, command, "
                                "Nova action, or recurring pattern nearby"})
...
def _timeline(ev: dict) -> list[dict]:
    ...
    for act in ev["actions"]:
        items.append({"t": act["timestamp"], "src": "nova",
