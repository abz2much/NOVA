"""Nova Decision Record (v7.32.0) — one immutable record per proactive decision.

Every proactive decision Nova makes (an anticipation alert, an intrusion escalation,
an automation suggestion) is written here as a single row that separates:

  * observation    — the raw facts Nova saw (entities, times, presence), no judgement
  * interpretation — Nova's *reading* of those facts (what it concluded)
...
def calibration(db_path: Optional[str] = None, kind: Optional[str] = None,
    ...
    A judged record scores 1.0 when its outcome is OUTCOME_GOOD (Nova was right
