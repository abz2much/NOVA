"""Nova mode entry actions (v7.15.0).
...
def _movie_area(hass, nova_config, audio_routing) -> Optional[str]:
    area = nova_config.get("movie_area", "") or ""
    if area:
        return str(area)
    mp = nova_config.get("movie_media_player", "") or ""
    if mp:
...
async def apply_mode_entry(hass, mode: str) -> None:
    ...
    try:
        if mode != "movie":
            return
        from . import nova_config, audio_routing
        area = _movie_area(hass, nova_config, audio_routing)
        if not area:
            return
        try:
            pct = int(nova_config.get("movie_dim_pct", 15))
        except Exception:
            pct = 15
