...
class NovaCommandCenterNew extends HTMLElement {
  ...
  static SETTINGS_CARDS = [
    ...
    { id: "floor_plan_editor", group: "home", title: "Floor Plan Editor", real: true,
      desc: "Rooms, outdoor zones, property line, windows/doors/dormers, camera placement, and AI camera-coverage — full parity with Classic." },
  ...
  // Reuses Classic's NOVA3D engine directly via window.NOVA3D (see the
  // export added in nova-panel.js) rather than re-deriving the ~1000 lines
  // of isometric-projection geometry — this tab only shapes the same small
  // options object (spec/plan/elements/garage/lit/doors/theta) Classic
  // already builds, using floor_plan_rooms/floor_plan_elements data this
  // panel already reads for its own Floor Plan Editor.
  _resStyles() {
    return {
