...
Canonical source for the rotatable 3D residence model (`NOVA3D`) shown on the
...
- **`house3d_core.js`** — the model. A pure-geometry axonometric projection rendered
  ...
  (`window.NOVA3D`). API: `build(opts)`, `renderSVG(opts)`, `fixedBox(opts)`.
...
```
cd custom_components/nova/frontend/dev
python3 -m http.server 8099   # then open http://localhost:8099/nova_house3d.html
```
