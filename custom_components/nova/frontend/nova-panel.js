 ...
 * v7.101.9 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const NOVA3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.101.9) ----------
  ...
  // One rectangle for a rectangular footprint; several for an L/T. (v7.101.9)
  // x-intervals of an orthogonal room polygon at scanline y (bay-aware decomposition, v7.101.9).
  ...
  function extWallsPoly(L, rooms, z0, z1) {
    ...
    // ONE stroked quad, not one per room edge (that seam was the "break") (v7.101.9).
  ...
  // ----- generalized roofs over the derived footprint (v7.101.9) -----
  ...
  // the house and low at the outer edge, split into two door panels (v7.101.9).
  ...
  // A door on a footprint wall, open (swung) or closed, for placed exterior/cellar doors (v7.101.9).
  ...
  // between the two rooms the wall separates (v7.101.9).
  ...
  // editor's footprint + rooms + home type (v7.101.9).
  function buildExteriorFromPlan(opts, plan, minx, miny, maxx, maxy, ztop) {
    ...
    if (rt !== 'flat') {
      ...
      // roof — so each dormer lights for ITS room, not the whole floor (v7.101.9).
    ...
    // the slope (v7.101.9). Front/back 2F glazing stays lower: that face is roof, so it reads
    ...
    // basement exterior openings — walkout doors / egress windows at grade (v7.101.9)
    var hb = null;
  ...
  function buildFromPlan(opts) {
    ...
    if (floor !== 'all') {
      ...
      draw(pf);
      // interior doors on this floor's rooms, open/closed from their sensor (v7.101.9)
      var zf = _planZ(pf);
...
class NovaPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.101.9)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.101.9)
    ...
    this._camMode = "stream";      // stream → still → nova (v7.101.9 fallback chain)
    ...
    // Real-time entity subscriptions (v7.101.9) — a native state_changed feed
    ...
    // Sparklines (v7.101.9) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.101.9) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.101.9: goals card. Also fixes suggestions, which _data() never
  ...
  // ── UI localization (v7.101.9) ── post-render text swap. The panel renders in
  ...
  // Placed openings (windows/doors) — v7.101.9. Parallel to the room plan.
  _getFloorElements() {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.101.9).
  _planToFeet(plan) {
    const FT = 0.2, out = {};
  ...
  // Align nearly-touching room edges (3D only) so exterior walls render seamless (v7.101.9).
  _snapFeet(out) {
    const TOL = 1.0;   // feet
  ...
  // Placed openings -> feet, with open/closed resolved from each mapped sensor (v7.101.9).
  _elementsToFeet(raw) {
  ...
  // as you edit, before saving (v7.101.9).
  ...
  // Per-bay garage door open/closed from the split door_mapping slots (v7.101.9).
  _house3dGarage() {
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.101.9).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.101.9): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.101.9) — rebuild it
  ...
  // ---- Floor-plan real dimensions (v7.101.9) ----
...
`;
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // clipped (v7.101.9). Drag uses getScreenCTM so it adapts to any viewBox.
    ...
    // rooms can be kept within it (v7.101.9). Enclosed rooms only (not outdoor zones).
    ...
    // placed openings as wall markers (v7.101.9)
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.101.9: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.101.9) -->
...
${this._renderExcludedEntities(d)}
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.101.9) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- Nova CHARACTER + RESEARCH (v7.101.9) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.101.9 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
      ...
      <!-- INTRUSION / SECURITY + LOG moved to their own tab (v7.101.9) -->

      <!-- VOICE CONFIRMATION (v7.101.9) -->
      <div class="panel">
        <div class="head">
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.101.9: approval now installs the automation into HA directly.
    ...
    // Routine learning: add/remove specific opt-in entities (v7.101.9)
    ...
    // Voice Confirmation: announce test (v7.101.9)
    // Manual "Analyze Now" — force a pattern-analysis pass (bypasses only the
    // 6h throttle, not the data gate) and show the outcome. (v7.101.9)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.101.9)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.101.9)
    ...
    // matching translation file and re-renders so the switch is immediate (v7.101.9).
    ...
    // Room → speaker assignment dropdowns (v7.101.9)
    ...
    // General fallback speaker (v7.101.9)
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.101.9).
  ...
  _wireFloorPlanControls() {
    ...
    // Openings (windows/doors) — add / edit / remove (v7.101.9)
    ...
    // Highlight an opening's marker while you hover its row or pick its sensor (v7.101.9)
  ...
  _wireFloorPlanDrag() {
    ...
    svgEl.addEventListener("mousemove", (e) => {
      ...
      if (dragging.resize) {
        ...
        // the garage (x<0) and in front of the home (y<0). (v7.101.9)
  ...
  _camName(entity) {
    // Nova-only display name (v7.101.9): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.101.9).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.101.9): MJPEG stream → proxy stills →
        ...
        // v7.101.9: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "nova") {
        ...
        // v7.101.9: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── Intrusion Log + training (v7.101.9) ──
  ...
  // ── System Diagnostics — core service health (v7.101.9) ──
  ...
  // ── Optional ChromaDB vector backend — v7.101.9 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras Nova uses (v7.101.9).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.101.9: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camNovaFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.101.9: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.101.9) */
  ...
  /* AREA DETAIL DRILL-DOWN (v7.101.9) */
  .area-detail-overlay {
...
class NovaPanelShell extends HTMLElement {
  ...
  async _decide() {
    ...
    // Self-healing (v7.101.9): "Panel look" can now change from three
