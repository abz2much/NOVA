...
class NovaPanel extends HTMLElement {
  constructor() {
    ...
    this._camMode = "stream";      // stream → still → nova (v7.94.0 fallback chain)
...
${this._renderExcludedEntities(d)}
      ...
      <!-- Nova CHARACTER + RESEARCH (v7.94.0) -->
      <div class="panel">
        <div class="head">
  ...
  _wire() {
    ...
    // Room → speaker assignment dropdowns (v7.94.0)
    ...
    // General fallback speaker (v7.94.0)
  ...
  _camName(entity) {
    // Nova-only display name (v7.94.0): camera_names map → picker name →
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras Nova uses (v7.94.0).
...
class NovaPanelShell extends HTMLElement {
  ...
  async _decide() {
    ...
    // Self-healing (v7.94.0): "Panel look" can now change from three
