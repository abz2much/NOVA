"""
Nova — continued conversation / turn-taking (v6.88.0).

Natural turn-taking: after Nova asks something, keep the satellite listening
...
mic-only satellite whose reply plays on a separate speaker, Nova watches that
speaker and reopens the satellite mic only once it goes idle — so the mic doesn't
capture Nova's own reply.
...
def enabled() -> bool:
    ...
    try:
        ...
        return bool(nova_config.get("continued_conversation_enabled", False))
    except Exception:
        return False
...
# Nova's own reply. Instead Nova watches the reply speaker and reopens the
...
def speaker_reopen_enabled() -> bool:
    """Whether Nova drives the follow-up reopen itself, timed to the separate
    speaker finishing (default on). Set False to fall back to Home Assistant's
    built-in continue_conversation reopen."""
    try:
        from . import nova_config
        return bool(nova_config.get("continued_conversation_speaker_reopen", True))
    except Exception:
        return True
...
def _multi_satellite_enabled() -> bool:
    ...
    try:
        from . import nova_config
        return bool(nova_config.get("continued_conversation_multi_satellite", False))
    except Exception:
        return False
