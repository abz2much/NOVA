"""
Nova biometric & wellbeing context (v6.63.0).

Lets Nova "feel" the user's state by reading biometric entities a wearable
...
looks concerning, Nova's job is to gently suggest the person check with a real
...
for Nova's *own behavior* (e.g. suppress chatter when a sleep sensor says
...
# ── context signals (for Nova's own behavior — non-medical) ────────────────
...
def wellbeing_context(hass) -> dict:
    """A compact, non-clinical snapshot for Nova's context — what a wearable
