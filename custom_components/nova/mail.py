...
(host, port, username, folder, SSL) come from nova_config, resolved at call
...
DEFAULT_SECRET_KEY = "nova_imap_password"
...
def _cfg(key: str, default):
    """Call-time config resolution (nova_config.get at call, not import)."""
    try:
...
async def fetch_recent(hass, *, limit: int = 5, unread_only: bool = False,
    ...
    if not _cfg("imap_enabled", False):
        return {"error": ("email is not enabled — set imap_enabled and "
                          "imap_host/imap_user in Nova settings")}
