...
@dataclass
class Announcement:
    """A record of something Nova said."""
    timestamp: float
    entity_id: str
    category: str
    urgency: str
...
# Nova interrupts less after a run of dismissed/false alarms. Opt-in and
...
def _budget_multiplier() -> float:
    ...
    try:
        ...
        if not nova_config.get("adaptive_interruption_budget", False):
            return 1.0
    except Exception:
        return 1.0
    now = _now()
