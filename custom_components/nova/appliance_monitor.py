"""
Nova — Appliance Cycle Monitor (v5.7.05).

Watches power consumption sensors to infer when household tasks finish.
Announces completion through the Nova audio pipeline.
...
# Nova names an appliance only when the load actually matches a confirmed one,
...
async def _announce_done(sensor: _SensorState, appliance_label: str) -> None:
    """Announce appliance cycle completion through Nova audio pipeline."""
    hass = _MON.hass
    config = _MON.config
...
async def start(hass: HomeAssistant, config: dict, entry=None) -> None:
    ...
    _LOGGER.info(
        "Nova Appliance Monitor v5.7.05 started — %d source(s): "
