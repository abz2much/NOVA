"""
Nova persona voice — phrase variety with a coherent character.

The point of variety is not randomness for its own sake; it is that Nova never
...
servile — and deliberately PLAINER as the situation grows graver. Nova does not
...
a dependency-free leaf (stdlib only) so any module can speak in Nova's voice.
...
def _banter_from_cfg() -> None:
    ...
    try:
        ...
        val = nova_config.get("banter_level", None)
        if val is not None:
            set_banter(val)
    except Exception:
        pass
