...
def classify(candidate: Any, records: Iterable[Any]) -> dict[str, Any]:
    ...
    for record in records:
        ...
        label = {"entity_id": entity_id, "name": name}
    ...
    if exact:
        return {"status": "already_automated", "matches": exact,
                "reason": "an equivalent loaded Home Assistant automation exists"}
    if overlaps:
        return {"status": "possible_overlap", "matches": overlaps,
                "reason": "a loaded automation controls the same target"}
    if unknown:
        return {"status": "unknown_overlap", "matches": unknown,
                "reason": "an opaque automation references the same target"}
    return {"status": "new", "matches": [], "reason": "no overlap found"}
