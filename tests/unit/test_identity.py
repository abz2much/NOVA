...
def cfg(load, monkeypatch):
    """Controllable nova_config.get (defaults to passthrough)."""
    jc = load("nova_config")
    store = {}
...
def test_two_home_alone_is_ambiguous_unknown(identity, cfg, sigs, fake_hass):
    sigs["home"] = ["Username", "Username2"]   # a tie on weak priors → not confident
    ident = identity.resolve(fake_hass)
...
def test_voice_provider_ignored_when_flag_off(identity, cfg, sigs, fake_hass):
    ...
    identity.register_voice_provider(lambda hass, dev: {"Username2": 1.0})
    sigs["home"] = ["Username", "Username2"]
    ident = identity.resolve(fake_hass)
...
def test_voice_provider_used_when_enabled(identity, cfg, sigs, fake_hass):
    ...
    ident = identity.resolve(fake_hass)
    identity.register_voice_provider(None)
    assert ident.person == "Username2" and "voice" in ident.method
...
def test_room_votes_sole_person_in_room(identity, monkeypatch):
    _stub_room(identity, monkeypatch, {"camera.bedroom2_room": "Username3"},
               {"camera.bedroom2_room": "bedroom2_room"})
    votes = identity._room_votes(None, "bedroom2_room", __import__("time").time())
    assert "Username3" in votes
    assert votes["Username3"] >= identity._W_ROOM_SOLE * 0.25
...
def test_quick_identify_ambiguous_returns_candidates(identity, monkeypatch):
    ...
    monkeypatch.setattr(identity, "_home_people", lambda h: ["Username", "Username3"])
    out = identity.quick_identify(None)
    assert out.person == identity.UNKNOWN
    assert set(out.candidates) == {"Username", "Username3"}
