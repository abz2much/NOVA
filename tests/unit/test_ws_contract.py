...
The panel talks to the backend over a set of `nova/*` WebSocket commands. A
...
def _type_from_decorator(dec: ast.AST):
    """Pull the "nova/…" type out of a @websocket_command({...type...: "…"})
...
def _frontend() -> set:
    """The nova/* command types the panel actually SENDS (type: "nova/…")."""
    src = _PANEL_JS.read_text()
    return set(re.findall(r"""type:\s*["'](nova/[a-z_]+)["']""", src))
