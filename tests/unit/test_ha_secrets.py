...
def test_read_valid_yaml(hs, tmp_path):
    ...
    p.write_text("nova_imap_password: hunter2\nother_key: value\n")
    data = hs._read_secrets(p)
    assert data["nova_imap_password"] == "hunter2"
...
def test_get_secret_sync_present(hs, tmp_path):
    p = tmp_path / "secrets.yaml"
    p.write_text("nova_imap_password: pw\n")
    assert hs.get_secret_sync("nova_imap_password", path=p) == "pw"
...
async def test_async_get_secret_via_executor(hs, fake_hass, tmp_path, monkeypatch):
    p = tmp_path / "secrets.yaml"
    p.write_text("nova_imap_password: async_pw\n")
    monkeypatch.setattr(hs, "SECRETS_PATH", p)   # resolved at call time
    assert await hs.async_get_secret(fake_hass, "nova_imap_password") == "async_pw"
