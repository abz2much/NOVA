...
def test_store_and_read(pp, tmp_path):
    ...
    assert pp.store("username", "time_routine", "start the coffee",
                    data={"hour": 6}, confidence=0.8, occurrences=7, db_path=db) is True
    rows = pp.read(db_path=db)
    assert len(rows) == 1
    r = rows[0]
    assert r["person"] == "username" and r["description"] == "start the coffee"
...
def test_upsert_refreshes_in_place(pp, tmp_path):
    db = str(tmp_path / "p.db")
    pp.store("username", "time_routine", "start the coffee", confidence=0.5, occurrences=5, db_path=db)
    pp.store("username", "time_routine", "start the coffee", confidence=0.9, occurrences=9, db_path=db)
    rows = pp.read("username", db_path=db)
...
def test_normalizes_person(pp, tmp_path):
    db = str(tmp_path / "p.db")
    pp.store("Username Smith", "time_routine", "x", db_path=db)
    assert pp.read("username_smith", db_path=db)            # normalized key matches
    assert pp.read(db_path=db)[0]["person"] == "username_smith"
