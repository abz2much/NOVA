...
Corroborated motion when away fires exactly one alert, then Nova investigates
...
async def test_motion_far_from_breach_does_not_confirm(safety, fake_hass, clock, monkeypatch, load):
    ...
    # exactly the pattern behind a false alarm. Nova keeps watching, doesn't
...
async def test_notify_all_falls_back_when_no_devices(cc, fake_hass):
    ...
    assert ("notify", "fallback", {"message": "hi", "title": "Nova — Security Alert"}) \
...
async def test_no_response_while_active_sends_soft_alert(safety, fake_hass, clock, cc):
    ...
    # response timeout with NO confirming inward route, Nova sends a SOFT
...
# Frigate's person sensor is a good trigger but false-positives; Nova's own
...
def _vision_says(safety, monkeypatch, verdict):
    """Stub Nova's vision confirmation to return True/False/None."""
    async def _v(cam):
        return verdict
...
async def test_vision_confirms_person_escalates(safety, fake_hass, clock, monkeypatch):
    ...
    _vision_says(safety, monkeypatch, True)        # Nova's eyes agree
    clock["now"] += 20
    _motion(fake_hass, "binary_sensor.kitchen_motion")
