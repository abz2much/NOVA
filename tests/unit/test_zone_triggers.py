...
def test_trigger_for_presence_becomes_zone(pa):
    assert pa._trigger_for("person.username", "not_home") == {
        "platform": "zone", "entity_id": "person.username",
        "zone": "zone.home", "event": "leave"}
    assert pa._trigger_for("person.username", "home") == {
        "platform": "zone", "entity_id": "person.username",
        "zone": "zone.home", "event": "enter"}
    assert pa._trigger_for("device_tracker.username_phone", "not_home")["event"] == "leave"
...
def test_trigger_for_nonpresence_stays_state(pa):
    ...
    assert pa._trigger_for("person.username", "work")["platform"] == "state"
...
def test_generate_zone_leave_automation(pa):
    p = pa.DetectedPattern(
        ...
        entity_ids=["person.username", "cover.garage"],
        confidence=0.8, occurrences=9,
        details={"trigger": {"entity": "person.username", "state": "not_home"},
    ...
    assert auto["trigger"] == {"platform": "zone", "entity_id": "person.username",
                               "zone": "zone.home", "event": "leave"}
...
def test_zone_trigger_survives_normalize(pa):
    auto = pa.normalize_suggestion_automation(json.dumps({
        "alias": "x",
        "trigger": {"platform": "zone", "entity_id": "person.username",
                    "zone": "zone.home", "event": "leave"},
    ...
    assert t["zone"] == "zone.home" and t["entity_id"] == "person.username"
...
def test_departure_sequence_detected_and_emitted_as_zone(pa, tmp_path):
    ...
    for d in range(8):
        ...
        _ins(conn, "person.username", "not_home", t, old="home")
    ...
    m = [p for p in pats if p.details["trigger"]["entity"] == "person.username"
