...
@pytest.fixture
def modules(load):
    return load("agent"), load("llm_provider")
...
async def test_run_agent_uses_activity_wrapper_for_main_call(
    ...
    async def fake_chat_with_activity(
    ...
    monkeypatch.setattr(llm_provider, "chat_with_activity", fake_chat_with_activity)
