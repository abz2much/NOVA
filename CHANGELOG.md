...
- Fixed decision history failing to open on installs whose `decisions.db` predates the `ref` column.
- Learned aliases, ignore rules and autonomy grants are now saved atomically, and an unreadable state file is logged as corrupt instead of looking empty.

Paths, table and column names, stored data, backends and service, WebSocket, panel and assistant tool schemas are unchanged. Databases gain a small `nova_schema_components` table, which older releases ignore.
...
**Fixes**
- A door, window, generic opening, garage door or lock coming back from `unknown` or `unavailable` (after a Home Assistant restart, or when its integration or device recovers) is no longer treated as someone opening or unlocking it. It is not classified, reasoned about, announced, notified or learned from, and Sentinel's instant alerts skip it. Duration rules still see the entity's current state. Previously the local cognition layer re-admitted these transitions, which produced announcements like "the first floor windows have registered as open ... the sensor was unavailable until now".
- A smoke, carbon monoxide, gas or leak sensor recovering straight into its active state, or the selected alarm recovering into `triggered`, is still critical, and now reaches the pipeline even with the cognition layer turned off.
- Home Assistant's generic `opening` device class is classified like doors and windows, without asking the provider.
- An ordinary door, window or opening event stays silent while the household is present, decided without the provider: a person reading home, or, when no person state is readable, a currently occupied area. An alarm armed away or on vacation always means away, even when a person reads home. A person tracked away is never overridden by occupancy, and a disarmed alarm alone is not taken as someone being home. Hazards and a triggered alarm are never silenced this way.
- The observer's noise filter now drops `_w` (watts) only as an entity ID suffix. Entities such as `binary_sensor.first_floor_windows` or `binary_sensor.kitchen_water_leak` were previously dropped because their IDs contain `_w`; real changes on them now reach the pipeline, and a water leak recovering straight into its active state is critical even with the cognition layer off.

Service, WebSocket, panel and assistant-tool schemas, config keys and storage are unchanged.
...
- Discovers System Monitor's processor/memory/disk/pressure/temperature entities by integration ownership and internal type (`translation_key`, with a unique_id-prefix fallback) — never by friendly name or entity_id, both of which a user can freely rename or localize. A single unambiguous candidate can auto-map; more than one requires a manual pick in the new Settings → Host Health card; a disabled System Monitor entity (several are disabled by default) is reported with setup guidance, never silently force-enabled.
- Recommended readings: Processor use, Memory usage, Memory/IO Pressure Some & Full 60s Average, Disk usage. Optional: Processor temperature, Swap usage, CPU Pressure Some 60s Average, Load 5 min, Uptime. The accumulated PSI "total" entities are structurally excluded from every alert decision (filtered by `state_class`, not by name). Fahrenheit is converted to Celsius using the standard conversion formula; `unknown`/`unavailable`/non-numeric/non-finite/stale readings are all treated as unavailable, never as zero or a guess.
- A small deterministic evaluator (no LLM involved) requires a problem to persist across multiple consecutive valid samples — a brief spike never alerts — before the first alert, and a comparably stable recovery streak before announcing recovery. Persistence tracking lives only in memory: a restart or reload starts from zero rather than reconstructing false persistence from anything stored, and a manual remap mid-streak resets tracking for that metric rather than carrying an old source's history onto a new one. Conservative default thresholds (documented in `host_health.py`, e.g. 90% CPU/memory/disk, 80°C processor temperature — explicitly not claimed as a hardware-validated safe limit), a 10-minute default persistence window, and a 60-minute default cooldown between repeat alerts on the same unresolved problem.
- Folded into the existing `system_diagnostics` result as a structured Host Health section (overall state, available readings, active/persistent problems, missing-or-stale readings, newest sample time) — HOMER already had this tool from Phase 7, so it can now use host-health evidence when diagnosing slowness, with no new tool added and its eight-tool grant completely unchanged.
- Alerts require both host-health awareness AND a separate alerts toggle, and route through the exact pieces every other Nova proactive monitor already uses — `output_gate` (rate limit/dedup), `audio_routing`'s presence-aware occupied-room routing, and the existing multi-notification-target path — never a standalone alert dispatcher. Simultaneous problems combine into one notification; a delivery failure is logged and never causes a retry loop; host resource load is never treated as a household safety emergency (never a critical-urgency announcement).
- Settings → Host Health: master toggle, a separate alerts toggle (turning it off silences alerts immediately), a bounded recovery-announcement toggle, bounded persistence/cooldown numeric inputs, per-metric mapping status with a manual-selection dropdown where a metric is ambiguous, and live value/staleness display for whatever's currently active.

## [7.111.0] — HOMER, Nova's single diagnostic specialist (Phase 7)

Nova's existing `delegate_task` sub-agent machinery (capability groups, the depth-1 cap, the actuator denylist) now also supports a named specialist profile: HOMER, a tightly bounded, read-only diagnostic specialist built on that same existing system — not a new engine, and not a second one running alongside the old `"diagnostics"` capability group. That group's tool list has been retired in favor of one canonical definition; `capability="diagnostics"` still works, as an alias that now resolves to the exact same grant as `profile="homer"`.
...
- A meaningful detection is normalised to one of five canonical labels (person, vehicle, animal, package, activity) and recorded as a synthetic `camera_event.<location>` row in the same pattern-learning store (`state_changes`) every other entity's history already goes through — not a parallel database. It can be used as a pattern *trigger* (e.g. "a person appears, then the porch light turns on") but can never become an action target; that boundary is enforced in code (`pattern_analyzer.service_for`, `agent.py`'s `execute_plan` domain allowlist), not only in prompt wording.
- Deduplicated by label, resolved location, and package sub-state within a short window (5 minutes by default) — the same detection seen by two integrations watching the same spot (Eufy and Frigate both catching a person at the front door, for example) creates one learning row, not two. A sensor already active when Nova starts is never recorded as a new event.
- A confidence floor applies only when a source actually supplies a numeric score (0–100 scale) — a native Eufy detection with no score is never rejected for lacking one, and a floor of 0 disables filtering entirely. Invalid, negative, non-finite, or out-of-range values are rejected safely rather than trusted.
- A resident is attributed to a person detection only when Nova's existing recognition cache has a fresh, sufficiently confident match for that specific camera — never inferred from "someone is home." Vehicle, animal, and package events are never attributed to anyone.
- Package events keep their delivered/stranded/taken (and mail) detail rather than flattening to a bare "package" state, so a suggested "packages normally arrive around this time" routine stays meaningful. Existing package announcements, notifications, and cooldowns are unchanged — this hooks the one existing choke point every real package-state transition already flows through.
- Nothing is ever stored beyond canonical label, camera/area, source, an optional confidence number, an optional resident name, and package state: no image, face embedding, raw provider payload, raw vision-model response, prompt, or credential.
- Respects Nova's existing learning setting — nothing is recorded while Nova's pattern-learning subsystem isn't running — plus a new, separate "Learn from camera detections" toggle under Settings → Learning & Memory → Routine Learning (on by default, with a clear opt-out).
...
Raising `classifier_rate_limit` yourself (Settings → Devices & Services → Nova → Configure) is still worth doing if you're hitting it often — this fix protects arrivals specifically, not everything else the cap was already dropping.
...
**3D house still rendered in Classic's cyan**: Command Center's Residence tab has reused Classic's `NOVA3D` engine directly since it first shipped there (v7.101.24) — including that engine's hardcoded cyan palette (`#00f2fe`-family), which matched Classic's own cyan theme but has looked out of place against Command Center's ember/gold chrome ever since, only now noticeable with Classic gone as the point of comparison. Retthemed every color in the engine — the shared `C` palette object plus roughly 20 stray hardcoded `rgba()`/hex literals scattered through individual rendering functions (dormers, chimneys, cased-opening fill, the ambient background glow, room-label text colors) that bypassed `C` entirely — to ember/gold, keeping the exact same alpha/opacity structure per state so occupancy-contrast logic is unchanged. The "dominant"/mmWave-detecting green stays green, consistent with the rest of the app's own status-dot language. Verified visually before shipping, same render-and-screenshot technique as the rest of tonight — confirmed no cyan remains anywhere in the rendered house, dormers included.
...
- Removed the `ui_style` config option entirely: the "Panel look" setting in the HA Configure dialog, the in-panel "Look" dropdown, and the backend plumbing for both (`const.py`, `config_flow.py`, `websocket.py`, `panel_register.py`, `strings.json` + all 7 translation files).
- `scripts/smoke_panel.js` rewritten: removed ~840 lines of Classic-only tests and ~30 lines testing the now-deleted style switcher. The surviving Command Center test suite (145 assertions) already covered the same functionality through Command Center's own UI, so nothing lost real coverage — three tests turned out to implicitly depend on now-deleted Classic tests' side effects on shared mock fixtures (`pattern_include_entities`, `_bioEnabled`, pending-facts queue); made each section reset its own starting state explicitly instead.
- `bump_version.sh` updated for the new file structure (see below).

**Known, deliberate capability loss**: Classic's Floor Plan Editor had an Export/Import button for a floor plan's JSON layout (manual backup/restore, or copying a layout between installs) that was never ported to Command Center — missed in the earlier parity audits since nothing in Command Center's own test suite exercised it, only Classic's. Not re-added; flagging it here in case it turns out to matter. Everything else audited function-by-function against Classic before deletion, including a bug-in-the-audit-tool near-miss: `bump_version.sh` was blindly rewriting 89 unrelated dated comments (like "this code was added in v7.101.28") on every past release — fixed in 7.101.29, unrelated to this release but caught during this same push toward deleting Classic.

## [7.101.29] — Command Center's Floor Plan Editor can upload a background image directly

Auditing Command Center for anything still missing before Classic can be deleted surfaced one real gap: its Floor Plan Editor had an opacity slider for an imported floor-plan background image, but no way to actually upload one — that still required switching to Classic. Added an upload button + hidden file input, wired the same way Classic's does (`FileReader` → data URL → `floor_plan_bg` for the current floor via `nova/update_config`). Command Center's Floor Plan Editor is now genuinely feature-complete against Classic — not yet confirmed in a live browser (file-picker/FileReader interaction can't run under jsdom), verify by uploading a real image once installed.
...
Also fixed along the way: jsdom's `window.eval` doesn't resolve bare `prompt()`/`confirm()` as global identifiers (a real quirk, not a typo) — the new Add Room/delete-room code calls `window.prompt()`/`window.confirm()` explicitly, and the smoke-test harness now stubs `window.prompt` the same way it already stubbed `window.confirm`.
...
Real bug caught live: the arrival welcome briefing fired the instant a person's GPS/zone flipped to "home" — often while they were still in the driveway or car, well before they'd actually walked in. New setting (Settings → Briefings → Arrival) lets you pick your front door's contact sensor; presence now only marks a *pending* arrival, and the briefing itself fires once that door actually opens (within a 10-minute window — a stale pending arrival, e.g. they parked in the garage instead, drops silently rather than announcing late or out of context). No front door configured means arrival briefings stay off entirely — a deliberate choice over silently keeping the old premature behavior as a fallback. The briefing content itself (weather/calendar/energy/etc.) is unchanged for now; only the trigger timing changed.

## [7.101.8] — doorbell training log now records what Nova would have said

`speak` — the exact line the vision model generates for Nova to say aloud — was computed on every notable camera event but never persisted anywhere; only `summary`/`analysis`/`category`/`notable` were logged. With `announcements_enabled` off (or during quiet hours), there was no way to see after the fact what a notable event would have sounded like — caught live while testing the Eufy doorbell rebuild with announcements off. `doorbell_training.log_event()` now stores `speak` (empty string when there wasn't one, e.g. the no-vision-call known-resident path), and the new look's Doorbell Training card shows it as a quoted line under each event.
...
- New `eufy.py`: discovers a camera's sibling sensors (ringing, person/stranger/identity detected, pet/crying/sound detected, package delivered/stranded/taken, the event-image entity) by **unique_id**, not entity_id — immutable, survives a user renaming the device, unlike matching on entity_id text.
- `__init__.py` gains a native state-change listener (not a bus-event one) covering every Eufy camera Nova knows about: doorbell ring gets the same full vision-analysis path a Nest chime gets; `stranger_person_detected` (Eufy's own unfamiliar-face flag — more reliable than an LLM guessing from a photo) triggers the existing silent visitor-learning vision call; a plain `person_detected` (a known/regular face) just logs directly with **no vision-LLM call at all**, since Eufy already told us it's not a stranger.
...
Several real gaps in the new Command Center look, all on the dashboard: Solar was fetched every poll but never actually rendered anywhere (dead code — the card just didn't exist), Quick Actions (Briefing, Nap, Unshush, Status Dump, Analyze Now) never got ported from Classic at all, the Areas grid was paired side-by-side with Activity in a 2-column layout that left it too narrow to lay out sensibly (now full-width, fixed 7 columns like Classic, so a typical 14-area house reads as two rows of 7), and Activity was double-capped at 6 events (a 2-hour/8-event fetch, then sliced to 6 again) with no way to see more — now pulls 24h/60 events and scrolls. Also added the same "Analyze Now" diagnostic Classic has (closest routine, days needed, busiest source) directly to the Suggestions tab's empty state, instead of leaving it undiscoverable on the Dashboard tab with no link from Suggestions.

## [7.101.1] — fix: sleep-state dropdown stuck after an override expired

The panel's sleep-state dropdown (Auto/Awake/Asleep) read the raw stored override value directly, with no expiry check — so once an "Awake"/"Asleep" override passed its auto-revert time (next quiet-hours end), the dropdown kept showing the stale choice forever instead of falling back to Auto, even though the underlying `is_sleeping()` suppression logic had already reverted correctly. New `sleep_detection.current_override()` computes the effective value (accounting for expiry) and the panel now reads that instead of the raw key.

## [7.101.0] — Settings UI for per-person honorifics (new look only)

Fourth and final release of the honorific rollout. A new **Person Honorifics** card (Settings → General, new Command Center look only) lists every `person.*` entity in the house, each with a dropdown (Sir/Ma'am/Boss/Friend, or "Custom…" for anything else) — picking one sets what Nova calls that person when they're home alone; "— use default —" removes the override and falls back to the global setting. Also converted the existing global "Address me as" field (Settings → Devices & Services → Nova → Configure, both the first-run screen and the Core options step) from free text to the same dropdown-with-custom-option pattern, using the `HONORIFIC_OPTIONS` constant that had sat unused since it was defined. `person_honorifics` was previously only settable by hand-editing `/config/nova/config.json`; Classic is untouched for this feature, per the plan to eventually retire it in favor of the new look.

## [7.100.0] — Nova now addresses whoever's actually home

Third and final behavior-changing release of the honorific rollout (the settings UI to configure per-person honorifics separately follows next). Every place Nova speaks — safety alerts, briefings, reminders, scenes, routines, lockdown/intrusion notices, sentinel watchdog alerts, camera prompts, and more — now resolves who to address at the moment of speaking instead of using one fixed global honorific: exactly one person home gets addressed as themselves (their own configured honorific, or the existing global default if they haven't set one); nobody home, or more than one person home, and Nova simply doesn't guess — the sentence drops the address entirely rather than picking a name at random. Two spots that used to compute the honorific once and hold onto it for the life of the object — the Sentinel watchdog and the infrastructure-audit monitor — were restructured to look it up fresh on every alert instead, since presence changes constantly over a session that can run for weeks. No settings change is needed for existing behavior to continue: with only one person ever home, or a single global honorific already in use, nothing sounds different. Per-person configuration (picking what Nova calls each household member) is the next release.

## [7.99.1] — Grammar refactor for presence-aware honorifics (still no behavior change yet)

Second of the multi-release honorific rollout. The honorific has always doubled as both the address term and the capital letter opening an announcement (`"Sir, the garage door is open."`) — across every safety notification (`notify_i18n.py`, all 7 languages), the persona voice module's phrase pools, and roughly 25 other modules that build spoken text (locks/lockdown, freeze/hazard alerts, briefings, reminders, scenes, routines, package/mail, appliance-cycle, camera/doorbell prompts, and more). This release makes every one of those sites safe to receive an empty honorific — "nobody specific home to address" — without producing a broken sentence like `", the garage door is open."`; `persona.py` gained a shared `lead_in()` helper for that, plus honorific-free variants in phrase pools that previously had none. Every current caller still always passes "sir" or the configured global default, so there is still no observable behavior change — this is purely the plumbing Phase C's real switch-over will rely on.

## [7.99.0] — Foundation for presence-aware honorifics (no behavior change yet)

First of a multi-release rollout: Nova currently addresses everyone with one global honorific ("sir" by default), used identically whether it's replying to a direct command or making a whole-house broadcast — wrong once more than one person lives there. This release lays the groundwork only — a new `honorific.py` module that knows who's home (reusing the same presence primitive `identity.py` already relies on) and can resolve a per-person honorific once one is configured, plus the config plumbing for a future "who should Nova call whom" settings card. Nothing in the app calls this yet, so there is no behavior change in this release — every announcement still says "sir" exactly as before. The actual switch-over (and the settings UI to configure it) follows in upcoming releases, done in phases specifically so the large grammar refactor it requires (honorific currently doubles as the capital letter that opens ~80 announcement sentences) can be verified safe before anything changes for real.

## [7.98.0] — Intrusion and Suggestions tabs — only Residence stays on Classic

Two more top-level tabs ported to the new look, both straight ports of Classic's own logic — no new behavior. **Intrusion**: last snapshot, ARMED/CALLED OFF status, "I'm looking" acknowledge and false-alarm call-off (safety-relevant, so kept byte-for-byte behaviorally identical to Classic), the auto-escalate timeout and vision-confirm settings, and a full intrusion log with real/false-alarm labeling that feeds Nova's damped-pattern learning. **Suggestions**: the learned-automation review queue — approve installs it into Home Assistant, dismiss clears it, "see the automation" reveals the YAML; nothing is ever created without approval. Suggestions rides on the same `nova/get_panel_data` payload the dashboard already polls, no new fetch.

With this, only **Residence** (the 3D house view) remains Classic-only — a plan for porting it later is written up in the vault, phased so a read-only 3D viewer (reusing the existing rendering engine near-verbatim) can ship well before the full drag-and-drop floor plan editor.
...
## [7.95.0] — the new look's Settings tab is (almost) all real now

25 of the new Command Center's 26 Settings cards are fully working, up from 2 — Residence/Home, Operational Mode, and Diagnostics finish General; AI Models, Briefings, Voice Confirmation, Satellite→Speaker, and Announcement Speakers finish Voice & Speakers; Notifications, Sentinel Rules, Hazard Monitor, Energy Management, and Appliances finish Awareness & Safety; Anticipation & Memory, Memory, Observer Tuning, Routine Learning, and Excluded Entities finish Learning & Memory; Cameras and Doorbell Training finish Cameras; and Wellbeing Context, Nova Character & Research, and Document Library finish Home & Extras. Each one mirrors Classic's own logic and websocket calls rather than introducing new behavior. Only the Floor Plan Editor stays a stub — a full drag-and-drop SVG room editor tied to Classic's Residence 3D view, out of scope here for the same reason that tab itself stays Classic-only. Along the way, fixed a real bug this surfaced: Routine Learning and Excluded Entities' add/remove controls weren't updating the in-memory config after a save, so a just-added or just-removed entry could appear to silently revert until the next full data refresh.
