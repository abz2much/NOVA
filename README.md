...
- HOMER, a named read-only diagnostic sub-agent (`delegate_task` profile `"homer"`): for a fault ("why is this unavailable," "why is Nova slow," "check connectivity and system health"), Nova can delegate to a focused specialist with a fixed diagnostic/telemetry/state tool set — no device control, configuration, memory-writing, or delegation tool — that separates observation from inference and reports a likely cause, its confidence, and a recommended next step back to Nova rather than acting on it. Unlike jarvis-aio's own version of this idea, which layers a directive on top of its standard prompt while that prompt still unconditionally claims the agent "has tools to control devices," HOMER gets a genuinely separate system prompt with no such claim — a sub-agent that can't actuate never tells the model it can. Always on, capped at 4 tool turns, and — like every sub-agent — unable to delegate further.
