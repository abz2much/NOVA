...
- A second, optional Command Center look (Settings → Devices & Services → Nova → Configure → "Panel look") — an animated "stellar core" centerpiece instead of a camera feed, so it looks and feels the same whether you have zero cameras or twelve, with its own navigation and Logs, Memory, Intrusion, and Suggestions tabs, plus a Settings tab reorganized around what you're doing rather than which subsystem it touches. Its Areas cards show every monitored room (no hard cap), each with capability icons, live temperature/humidity sparklines, and a light toggle. All 26 settings cards are now functional, matching Classic feature-for-feature — including the Floor Plan Editor (rooms, outdoor zones, property-line boundary, camera placement, and AI camera-coverage estimation), though window/door placement within it still links out to Classic for now. Only the Residence 3D view remains fully Classic-only. Classic stays the default; nothing is deleted.
