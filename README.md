...
- A second, optional Command Center look (Settings → Devices & Services → Nova → Configure → "Panel look") — an animated "stellar core" centerpiece instead of a camera feed, so it looks and feels the same whether you have zero cameras or twelve, now with its own navigation and a Settings tab reorganized around what you're doing rather than which subsystem it touches, plus a settings search box. Classic stays the default; nothing is deleted, and unbuilt settings are labeled honestly rather than missing.
