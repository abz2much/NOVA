...
- A second, optional Command Center look (Settings → Devices & Services → Nova → Configure → "Panel look") — an animated "stellar core" centerpiece instead of a camera feed, so it looks and feels the same whether you have zero cameras or twelve, with its own navigation, Logs and Memory tabs, and a Settings tab reorganized around what you're doing rather than which subsystem it touches, plus a settings search box. 25 of its 26 settings cards are now fully functional, matching Classic feature-for-feature; only the Floor Plan Editor (a full drag-and-drop SVG room layout tool) stays on Classic for now, the same reason the Residence 3D tab itself does. Only Residence, Intrusion, and Suggestions remain Classic-only. Classic stays the default; nothing is deleted.
